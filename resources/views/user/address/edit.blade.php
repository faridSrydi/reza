...
@section('user_content')
    <div class="w-full">
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10">
            <div>
                <h1 class="text-4xl lg:text-5xl font-bold serif-text mb-2">Edit Address</h1>
                <p class="text-gray-500 dark:text-gray-400">Update your shipping address details.</p>
            </div>
            ...
            <form action="{{ route('addresses.update', $address->id) }}" method="POST" class="space-y-6">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <textarea id="address" name="address" rows="4" required
                        class="form-input w-full px-4 py-3 rounded-lg bg-[#f8f6f7] dark:bg-[#2d1622] border-[#f3e7ed] dark:border-[#3d2030] text-sm focus:ring-primary">{{ old('address', $address->address ?? '') }}</textarea>
                </div>
