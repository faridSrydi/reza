...
@section('content')
    <main class="flex-1 flex flex-col items-center py-10 px-4 relative overflow-hidden min-h-screen">
        ...
        <div class="absolute bottom-32 right-10 opacity-10 -rotate-12 pointer-events-none">
            <span class="material-symbols-outlined text-[100px] text-primary">category</span>
        </div>
        ...
        <div class="w-full max-w-[760px] z-10">
            ...
            {{-- HEADER --}}
            <div class="mb-8">
                <h1 class="text-[#181113] tracking-tight text-4xl font-black leading-tight text-center">Edit Category</h1>
                <p class="text-primary/70 text-center mt-2 font-medium">ID: {{ substr(md5($category->id), 0, 8) }} · Sweet tweaks ✨</p>
            </div>

            {{-- FORM CONTAINER --}}
            <div class="bg-white rounded-[3rem] border-[6px] border-primary shadow-2xl p-8 md:p-12 mb-20 relative">
                <form method="POST" enctype="multipart/form-data" action="{{ route('admin.categories.update', $category) }}" class="space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-[160px_1fr] gap-4 items-start">
                            <div class="w-40 h-40 rounded-[2rem] border-4 border-primary/10 bg-primary/5 overflow-hidden flex items-center justify-center">
                                @if ($category->image)
                                    <img id="categoryImagePreview" class="w-full h-full object-cover" alt="Preview" src="{{ asset('storage/' . ltrim($category->image, '/')) }}" />
                                    <span id="categoryImagePlaceholder" class="hidden text-primary/50 font-bold text-xs">No photo</span>
                                @else
