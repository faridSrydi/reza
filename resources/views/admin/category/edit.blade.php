...
<header class="lg:sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-[#f3e7ed] px-4 sm:px-6 lg:px-8 py-4">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-3xl font-bold serif-text">Edit Category</h1>
            <p class="text-sm text-gray-500 mt-1">Slug: <span class="font-mono">{{ $category->slug }}</span></p>
        </div>
...
<main class="p-4 sm:p-6 lg:p-8">
    ...
    <form id="categoryForm" method="POST" enctype="multipart/form-data" action="{{ route('admin.categories.update', $category) }}">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-[160px_1fr] gap-6 items-start">
                        <div class="w-40 h-40 rounded-xl border border-[#f3e7ed] bg-[#fcfaff] overflow-hidden flex items-center justify-center">
                            @if ($category->image)
                                ...
                                <span id="categoryImagePlaceholder" class="hidden text-gray-400 text-xs font-bold">No photo</span>
                            @else
            ...
            <div class="space-y-8">
                <section class="form-section-card">
                    ...
                    <p class="text-sm text-gray-600">Slug updates automatically when you change the name.</p>
                </section>
            </div>
        </div>
